...
class TimelineController extends AppBaseController
{
  ...
  public function createShare(Request $request)
    {
        ...
        $share->notifications_user()->sync([Auth::user()->id], true);
        ...
        if ($share) {
            ...
            foreach ($usernames[2] as $value) {
                $timeline = Timeline::where('username', str_replace('@', '', $value))->first();
                $notification = Notification::create(['user_id' => $timeline->user->id, 'post_id' => $share->id, 'notified_by' => Auth::user()->id, 'description' => Auth::user()->name.' '.trans('common.mentioned_you_in_post'), 'type' => 'mention', 'link' => 'share/'.$share->id]);
            }
            $timeline = Timeline::where('id', $request->timeline_id)->first();
            ...
            // Let us tag the post friends :)
            if ($request->user_tags != null) {
                $share->users_tagged()->sync(explode(',', $request->user_tags));
            }
        }
 ...
 public function getGroupShares($username, $group_id)
    {
        $user_post = 'group';
        $timeline = Timeline::where('username', $username)->with('groups')->first();
        $shares = $timeline->shares()->where('approve', 1)->orderBy('created_at', 'desc')->with('comments')->get();
        $group = $timeline->groups;
        $group_members = $group->members();
        $group_events = $group->getEvents($group->id);
        $ongoing_events = $group->getOnGoingEvents($group->id);
        ...
        return $theme->scope('timeline/groupshares', compact('timeline', 'group', 'shares', 'group_members', 'next_page_url', 'user_shares', 'username', 'group_events', 'ongoing_events', 'upcoming_events'))->render();
    }
