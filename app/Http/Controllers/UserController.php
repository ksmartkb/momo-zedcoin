...
class UserController extends AppBaseController
{
    ...
    public function zedcoinAccount($username)
    {
        $timeline = Timeline::where('username', $username)->with('user', 'user.pageLikes', 'user.groups')->first();
        $user = $timeline->user;
        $zedcoin_amount = DB::table('zedcoin_account')->where('user_id', Auth::user()->id)->first();
        $admin_role_id = Role::where('name', '=', 'admin')->first();
        $joined_groups = $user->groups()->where('role_id', '!=', $admin_role_id->id)->where('status', '=', 'approved')->get();
        $joined_groups_count = $joined_groups->count();
        $following_count = $user->following()->where('status', '=', 'approved')->get()->count();
    ...
    public function mtnTransactionCollection(Request $request){
        //method to make collection/ this is the method users use to make payment to the zedcoin account so that they start saving there money on the app.
        $transaction_id = 122345545; // transaction id - can be anynumber preferebly that makes sense - like contains date and user id
        $phone = 46733123450;//user phone already predifined by user (mtn phone number)
        $amount = 2021; //amount to deposit by user
        $collection = new Collection(); // am using brian matovu library to connect to the mtn api
        $mtnTransactionId = $collection ->transact($transaction_id,$phone,$amount);
        $result  = $collection->getTransactionStatus($mtnTransactionId);
      dd($result);
